...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/MirvDesk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"MirvDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon MirvDesk.app 200 190 --hide-extension MirvDesk.app mirvdesk.dmg ./build/macos/Build/Products/Release/MirvDesk.app")
    os.rename("mirvdesk.dmg", f"../mirvdesk-{version}.dmg")
    '''
    os.chdir("..")
...
def main():
    ...
    if windows:
        ...
        if flutter:
            ...
            if osx:
                system2(
                    'strip target/release/bundle/osx/MirvDesk.app/Contents/MacOS/rustdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/MirvDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/MirvDesk.app/Contents/MacOS/rustdesk
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/MirvDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/MirvDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/MirvDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/MirvDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "MirvDesk %s.dmg" "target/release/bundle/osx/MirvDesk.app"' % version)
                os.rename('MirvDesk %s.dmg' %
                          version, 'mirvdesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./mirvdesk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./mirvdesk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple mirvdesk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/MirvDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
