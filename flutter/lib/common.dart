...
class MyTheme {
  ...
  static const Color accent = Color(0xFF0097A0);
  static const Color accent50 = Color(0x770097A0);
  static const Color accent80 = Color(0xAA0097A0);
  ...
  static const Color button = Color(0xFF00A3AD);
  ...
  if (uri.authority.isEmpty &&
    ...
    if (isAndroid || isIOS) {
      ...
      if (!allowDeepLinkServerSettings) {
        debugPrint(
            "Ignore ${bind.mainUriPrefixSync()}config because $kOptionAllowDeepLinkServerSettings is not enabled.");
    ...
    if (isAndroid || isIOS) {
      ...
      if (!allowDeepLinkPassword) {
        debugPrint(
            "Ignore ${bind.mainUriPrefixSync()}password because $kOptionAllowDeepLinkPassword is not enabled.");
        // Keep the user-facing error generic; detailed rejection reason is in debug logs.
        // Delay toast to avoid missing overlay during cold-start deeplink handling.
        Timer(Duration(seconds: 1), () {
          showToast(translate('Failed'));
    ...
    // mirvdesk://<connect-id>
    // mirvdesk://<connect-id>/r
    // mirvdesk://<connect-id>/r@<server>
    command = '--connect';
    id = uri.authority;
    if (uri.path.length > 1) {
      id = id + uri.path;
